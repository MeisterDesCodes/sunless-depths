...
enum resourceType {RESOURCE, MATERIAL, CONSUMABLE, BLUEPRINT, WEAPON, AMMUNITION, EQUIPMENT, NONE, MAP_LOCATION, MAP_PATHWAY, STATUS_EFFECT, CURIOSITY}

enum weaponAttackType {BASIC_MELEE_SWING_FRONT, BASIC_MELEE_SWING_BACK, BASIC_MELEE_STAB, BASIC_RANGED}

enum enemyStates {IDLE, CHASE, DASH, KEEP_DISTANCE, LAUNCH_PROJECTILE, DEATH}
...
enum statusEffectType {BLEED, POISON, BURN, ON_HIT, HEAL, STAMINA_COST_REDUCTION, SLOWNESS, AWARENESS, QUICK_FEET, HASTE, \
	MELEE_DAMAGE_INCREASE, RANGED_DAMAGE_INCREASE, DAMAGE_INCREASE, STAMINA_RESTORE, STAT_INCREASE, SATURATION, \
	STARVATION, SUFFOCATION}

enum statusEffectReceiver {SELF, TARGET}

enum locations {NONE, A_PAST_FORGOTTEN, THE_SETTLEMENT, CENTRAL_PATHWAY, EASTWARD_QUARRIES_1, EASTWARD_QUARRIES_2, EASTWARD_QUARRIES_3, OLD_TOMS_WORKSHOP}
...
enum caveVariations {NONE, OLD_TOMS_WORKSHOP}
